...
export async function activateWarrantyByPurchaseMatch(body) {
  ...
  for (const [purchaseId, purchase] of Object.entries(purchases)) {
    ...
    if (
      ...
      status !== "activated" &&
      normalizeWarranty(purchase?.warranty_number)
    ) {
      matches.push({
        purchaseId,
        purchase,
        sku,
        warrantyNumber: normalizeWarranty(purchase.warranty_number)
      });
    }
  }
  ...
  if (warrantyRecord?.status === "activated") {
    throw httpError(409, "Phiếu bảo hành này đã được kích hoạt trước đó.");
  }
